...
@testset "TestBlockInterface Implementation Tests" begin
    ...
    @testset "prepend_preamble_statements function" begin
        ...
        result_with = prepend_preamble_statements(
            interface_with_preamble, existing_preambles
        )
        ...
        result_without = prepend_preamble_statements(
            interface_without_preamble, existing_preambles
        )
    ...
    @testset "Edge Cases" begin
        ...
        try
            ...
            if !isnothing(JuliaSyntax.children(root)) &&
                !isempty(JuliaSyntax.children(root))
                node = only(JuliaSyntax.children(root))
